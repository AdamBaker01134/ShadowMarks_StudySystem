...
StreamData2 <- read_csv("shadowmarksStreamData_2024-08-19.csv", col_names=TRUE)
StreamData2
...
StreamData2 <- StreamData2 %>% filter(!(pID %in% complete_outliers)) %>%
  filter(event=="added_mark") %>% group_by(pID,condition,interaction,trial,attempt,selectedVideos,event) %>% summarise()
...
DifferenceData2 <- Data2 %>% filter(!(pID %in% complete_outliers)) %>%
  group_by(interaction,trial) %>% summarise(mean=mean(difference2,na.rm=TRUE),
                                            sd=sd(difference2,na.rm=TRUE),
                                            se=sd/sqrt(length(difference2)))
...
DifferenceData2 <- Data2 %>% filter(!(pID %in% complete_outliers)) %>%
  group_by(interaction,trial) %>% summarise(mean=mean((difference1+difference2)/2,na.rm=TRUE),
