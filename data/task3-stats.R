...
DataTooLong <- DataFull %>% filter(incompleteTrial) %>% group_by(pID,technique,task,trial) %>% summarise(elapsedTime=max(elapsedTime)/1000, errors=max(attempt)-1)
DataTooLong

# Counting number of trials removed
DataTooLongRemove <- DataFull %>% filter(incompleteTrial) %>% group_by(pID,technique,task,trial,attempt) %>% summarise(elapsedTime=max(elapsedTime)/1000, errors=max(attempt)-1) %>% filter(attempt==1)
DataTooLongRemove
...
DataFull <- DataFull %>% filter(!incompleteTrial)
...
ggplot(ctSummary, aes(x=technique, y=mean)) +
  ...
  geom_label(aes(label = paste(round(mean,digits=2))), nudge_y = c(12.5,13,11), size = 4,
...
ggsave("task3/ct-by-technique-task3.png", width=10, height=10, units="cm", type="cairo-png")
...
ggplot(accSummary, aes(x=technique, y=mean)) +
  geom_col(fill=c("#FF9980","#99CCFF","#00C000"), color="black", width=0.5) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width = 0.2) +
  geom_label(aes(label = paste(round(mean,digits=2))), nudge_y = c(0.5,0.5,0.45), size = 4,
             label.size = 0, label.r = unit(0, "pt")) +
  new_theme +
  scale_x_discrete(limits=c("smallMultiples","overlays","shadowMarkers"),
                   labels=c("Small-\nMultiples","Overlays","Shadow\nMarks")) +
...
ggsave("task3/acc-by-technique-task3.png", width=10, height=10, units="cm", type="cairo-png")
...
ggplot(DifferenceData, aes(x=technique, y=mean)) +
  geom_col(fill=c("#FF9980","#99CCFF","#00C000"), color="black", width=0.5) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width = 0.2) +
  geom_label(aes(label = paste(round(mean,digits=2))), nudge_y = c(2.2,2.5,2.5), size = 4,
             label.size = 0, label.r = unit(0, "pt")) +
  new_theme +
  scale_x_discrete(limits=c("smallMultiples","overlays","shadowMarkers"),
                   labels=c("Small-\nMultiples","Overlays","Shadow\nMarks")) +
...
ggsave("task3/error-distance-by-technique-task3.png", width=10, height=10, units="cm", type="cairo-png")
...
#   guides(fill=guide_legend(title=NULL))
# 
# ggsave("task3/error-distance-per-trial-task3.png", width=20, height=20, units="cm", type="cairo-png")
